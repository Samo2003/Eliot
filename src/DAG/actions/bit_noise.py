...
class BitNoise(ActionBase[Literal["BitNoise"]]):
    ...
    # Offset in bits where to start inverting bits (if negative bits are counted from the end)
    start: int | ValueGeneratorInt = 0

    # Offset in bits where to stop inverting bits (if negative bits are counted from the end)
